...
#Return mixture of name and initials and mixture of two charecter initials
def complex_initials(name):

	if ' ' in name:
		name = name.title()
		out_phrases = []
		name = name.split()
		S = set(list(range(len(name))))
		for i in range(len(name)):
			if i == 0:
				continue
			list_name = list(itertools.combinations(S, i+1))
			for j in range(len(list_name)):
				for z in range(i+1):
					result = ""
					for w in range(i+1):
						if w != z:
							result += name[list_name[j][w]][0]
						else:
							result += name[list_name[j][z]]
					out_phrases.append(result)
					out_phrases.append(result.lower())
		result = ""
		for i in range(len(name)):
			result += name[i][0]+name[i][1]
		out_phrases.append(result)
		out_phrases.append(result.lower())

		return list(set(out_phrases))
...
class MainSubject(Person):
 ...
 """)
	...
	def main_sub_outputWords(self):
		...
		if len(self.initials) !=0 : word_output.extend(complex_initials(self.name))
...
def getMoreNumbers():
 ...
 """)
	...
	# Famous numbers that people use for their passwords
	add_nums.append("1")
	add_nums.append("123")
	add_nums.append("123456")
	add_nums.append("12345")
	add_nums.append("12345678")
	add_nums.append("111")
	add_nums.append("111111")
	add_nums.append("2")
	add_nums.append("3")
	add_nums.append("4")
	add_nums.append("5")
